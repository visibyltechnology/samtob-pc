...
// Import your Save to Buy component or logic here if applicable
...
export default function CheckoutPage() {
  ...
  // Added "save-to-buy" to the payment state type
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      } else if (payment === "save-to-buy") {
        // Handle Save to Buy routing (e.g., redirect to a savings plan dashboard or confirmation)
        clear();
        router.push(`/save-to-buy/success/${order.id}`);
      } else {
      ...
      >
            ...
            <div className="grid gap-3">
              ...
              >
                <input
                  ...
                  onChange={() => setPayment("save-to-buy")}
                  className="mt-0.5 accent-[var(--color-signal)]"
                />
                <span>
                  <span className="block font-medium">
                  ...
                  <span className="text-xs text-steel">
                    Save towards this item at your own pace. Item ships once
                    fully paid.
                  </span>
                </span>
              </label>
        ...
        <div className="border border-line rounded-2xl p-6 h-fit sticky top-24">
            ...
            >
              {submitting
                ...
                : payment === "klump"
                  ? "Continue to Klump"
                  : payment === "save-to-buy"
                    ? "Start Save-to-Buy Plan"
                    : "Place Order"}
            </button>
          ) : (
            <div className="mt-5">
              <KlumpCheckoutButton
                amount={pendingOrder.total} // total already = subtotal + deliveryFee, that's fine
                shippingFee={deliveryFee} // NEW — must be passed so Klump's amount check passes
                items={items.map((i) => ({
                  unit_price: i.price,
                  quantity: i.qty,
                  name: i.name,
                ...
                customerEmail={email}
                customerFirstName={name.trim().split(" ")[0]} // NEW
                customerLastName={
          ...
          <p className="text-[11px] text-steel mt-3 leading-relaxed">
            Orders received before 12noon ship same day; after 4pm ship the next
            working day.
          </p>
        </div>
      </form>
